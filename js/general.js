...
function loadFavorite() {
    ...
    elmHeartList.forEach((element) => {
        if (
            favoritePostList.indexOf(parseInt(element.getAttribute("id"))) !==
            -1
        ) {
...
getWeather()
    .then(function (data) {
        getDayTextByCodeAndLang(data.current.condition.code, "vi").then(
            function (textConditon) {
